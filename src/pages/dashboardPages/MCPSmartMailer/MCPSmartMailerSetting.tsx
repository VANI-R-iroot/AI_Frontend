...
const McpServerSetup = () => {
  ...
  const [googleExcelIds, setGoogleExcelIds] = useState<string[]>([]);
  ...
  const addGoogleDocId = () => {
    if (newDocId.trim() && googleExcelIds.length < 5) {
      setGoogleExcelIds([...googleExcelIds, newDocId.trim()]);
      setNewDocId("");
    }
  };

  const removeGoogleDocId = (index: number) => {
    setGoogleExcelIds(googleExcelIds.filter((_, i) => i !== index));
  };

  const fetchActiveGoogleDocument = async () => {
    try {
      const res = await axiosInstance.get("/get-google-excel-id");

      if (res.status === 200 && res.data?.data?.length > 0) {
        const activeDocs = res.data.data[0].googleExcelIds;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!googleExcelIds || googleExcelIds.length === 0) {
      toast.error("Please add at least one Google Doc ID!");
      return;
    }

    if (googleExcelIds.length > 5) {
      toast.error("Maximum 5 Google Doc IDs allowed!");
      return;
    }

    const invalidIds = googleExcelIds.filter((id) => !isValidGoogleDocId(id));
    ...
    try {
      const payload = { googleExcelIds: googleExcelIds };
      const res = await axiosInstance.post("/save-google-excel-id", payload);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.put("/update-google-excel-id", payload);
  ...
  const handleDeleteActiveDoc = async (index: number) => {
    ...
    try {
      ...
      const res = await axiosInstance.post("/delete-google-excel-id", payload);
    ...
    <div className="mcp-container">
      ...
      <div className="mcp-personal-glass-card">
        <div className="card-header">
          ...
          <div>
            <h5 className="card-title">Google Excel Documents</h5>
            <p className="card-subtitle">
              Manage Your Google Excel IDs (Max: 5)
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <div className="mcp-server-input-group">
              <div className="mcp-personal-mcp-server-input-wrapper">
                <input
                  ...
                  placeholder="Enter Google Excel ID"
                ...
                >
                  ...
                  <div className={`tooltip ${showTooltip ? "show" : ""}`}>
                    <div className="tooltip-title">Google Excel ID Demo:</div>
              ...
              <button
                ...
                disabled={
                  !newDocId.trim() || googleExcelIds.length >= 5 || isSubmitting
                }
            ...
            {newDocId && !isValidGoogleDocId(newDocId) && (
              <div className="validation-message">
                <FiAlertCircle />
                <span>Please enter a valid Google Excel ID</span>
              </div>
            )}

            <div className="progress-indicator">
              {googleExcelIds.length}/5 documents added
            </div>
          </div>

          {googleExcelIds.length > 0 && (
            <div>
              <h6
                style={{
              ...
              <div className="mcp-doc-list">
                {googleExcelIds.map((docId, index) => (
          ...
          {googleExcelIds.length > 0 && (
            <div style={{ marginTop: "1.5rem" }}>
              <button
                type="submit"
                disabled={isSubmitting || googleExcelIds.length === 0}
                className="mcp-personal-mcp-server-btn mcp-btn-primary"
                style={{
                  width: "100%",
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    ...
                    Submit Google Excel IDs ({googleExcelIds.length})
                  </>
                )}
              </button>
            </div>
