...
const CustomTemplatePage = () => {
  ...
  const [chatAssistantList, setChatAssistantList] = useState<TemplateType[]>(
    []
  );
  const [filteredList, setFilteredList] = useState<TemplateType[]>([]);

  useEffect(() => {
    const fetchChatAssistantList = async () => {
      try {
        const res = await axiosInstance.get("/templateListAdmin");
        const chatAssistants = res.data.data;
    ...
    const storedData = sessionStorage.getItem("templateAdminList");
  ...
  useEffect(() => {
    ...
    const filtered = chatAssistantList.filter(
      (assistant) =>
        assistant.templateName?.toLowerCase().includes(lowercasedSearch) ||
  ...
  const toggleVisibility = async (id: string, currentStatus: string) => {
    ...
    try {
      await axiosInstance.post("/showHideTemplate", {
        id,
        bullionData: newStatus,
      });

      const res = await axiosInstance.get("/templateListAdmin");
      const updatedAssistants = res.data.data;
      sessionStorage.setItem(
        "templateAdminList",
  ...
  const handleDeleteAssistant = async (id: string) => {
    if (!window.confirm("Are you sure you want to delete this chat assistant?"))
      return;

    try {
      const res = await axiosInstance.post(`/deleteTemplate/${id}`);
  ...
  const columns: GridColDef[] = [
    { field: "templateIcon", headerName: "Icon", width: 150 },
    { field: "templateName", headerName: "Assistant Name", width: 200 },
